...
export default function MobileNav({ isOpen, setIsOpen }: MobileNavProps) {
  ...
  return (
    <>
      <nav className="md:hidden bg-white border-b border-[#EBECF0] p-4 flex items-center justify-between">
                  ...
                  {navItems.map((item) => (
                    <li key={item.href}>
                      <Link href={item.href}>
                        <a
                          className={cn(
                            "flex items-center px-4 py-3 rounded-md hover:bg-[#F4F5F7] transition-colors",
                            location === item.href
                              ? "text-[#0052CC] bg-[#F4F5F7] font-medium"
                              : "text-[#172B4D]"
                          )}
      ...
      <nav className="md:hidden bg-white border-t border-[#EBECF0] fixed bottom-0 left-0 right-0 flex justify-around z-10">
        {navItems.slice(0, 4).map((item) => (
          <Link key={item.href} href={item.href}>
            <a className={cn(
              "p-3 flex flex-col items-center",
              location === item.href ? "text-[#0052CC]" : "text-gray-500"
            )}>
